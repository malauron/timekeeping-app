...
Public Class frmAssignSchedules
    ...
    Private Sub btnBrowseCutoffPeriod_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnBrowseCutoffPeriod.Click
        ...
        If mSearchID > 0 Then
            ...
            If NetOpen(dtCutoffPeriods, "SELECT CONCAT(DATE_FORMAT(start_date,'%m/%d/%Y'),' - ',DATE_FORMAT(end_date,'%m/%d/%Y')) description,start_date,end_date FROM cutoff_periods where cutoff_period_id =" & mSearchID) = True Then
                If dtCutoffPeriods.Rows.Count > 0 Then
                    For Each drCutoffPeriod As DataRow In dtCutoffPeriods.Rows
                        ...
                        mDateFrom = CType(drCutoffPeriod.Item("start_date"), Date)
                        mDateUntil = CType(drCutoffPeriod.Item("end_date"), Date)
                        If CType(txtCutoffPeriod.Tag, Integer) > 0 And CType(txtEmployeeName.Tag, Integer) > 0 Then
                            Call Load_Schedules()
                        End If
                    Next
    ...
    Private Sub Load_Schedules()
        ...
        Dim mDummyStartDate As Date

        mDummyStartDate = mDateFrom
        mString = "(select " & CType(txtEmployeeName.Tag, String) & " employee_id,'" & Format(mDateFrom, "yyyy-MM-dd") & "' work_date "
        Do While mDummyStartDate < mDateUntil
            mDummyStartDate = DateAdd(DateInterval.Day, 1, mDummyStartDate)
            mString = mString & " union all select " & CType(txtEmployeeName.Tag, String) & " employee_id,'" & Format(mDummyStartDate, "yyyy-MM-dd") & "' work_date"
        Loop
        mString = mString & ")"
        If NetOpen(mAssignSchedules, " select a.*,if(isnull(b.schedule_id),0,b.schedule_id) schedule_id, " & _
                    ...
                    "from " & mString & " a " & _
                    "left outer join (select * from schedules_assignment where work_date between '" & Format(mDateFrom, "yyyy-MM-dd") & "' and '" & Format(mDateUntil, "yyyy-MM-dd") & "') b on a.employee_id=b.employee_id and a.work_date=b.work_date " & _
                    "left outer join schedules c on b.schedule_id=c.schedule_id " & _
                    "left outer join (select * from employee_dtrs where work_date between '" & Format(mDateFrom, "yyyy-MM-dd") & "' and '" & Format(mDateUntil, "yyyy-MM-dd") & "') d on a.employee_id=d.employee_id and a.work_date=d.work_date " & _
                    "LEFT OUTER JOIN (select * from employee_dtr_adjustments where work_date between '" & Format(mDateFrom, "yyyy-MM-dd") & "' and '" & Format(mDateUntil, "yyyy-MM-dd") & "') e ON a.employee_id=e.employee_id AND a.work_date=e.work_date", Cn.Connection) = True Then
