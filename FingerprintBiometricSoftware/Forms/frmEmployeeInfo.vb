...
Public Class frmEmployeeInfo
    ...
    Private Sub Clear_Fields()
        ...
        If SysParam.AddEMployee = 0 Then
            ...
            If NetOpen(dtEmployees, "select a.*,b.description,c.description department_name from employees a " & _
                ...
                If dtEmployees.Rows.Count > 0 Then
                    ...
                    For Each drEmployee As DataRow In dtEmployees.Rows
                        ...
                        txtSchedule.Tag = CType(IIf(IsDBNull(drEmployee.Item("schedule_id")), "", drEmployee.Item("schedule_id")), String)
                        txtDepartment.Text = CType(IIf(IsDBNull(drEmployee.Item("department_name")), "", drEmployee.Item("department_name")), String)
                        txtDepartment.Tag = CType(IIf(IsDBNull(drEmployee.Item("department_id")), "", drEmployee.Item("department_id")), String)
                        radDaily.Checked = CBool(IIf(CType(drEmployee.Item("pay_type"), String) = "D", True, False))
                        radMonthly.Checked = CBool(IIf(CType(drEmployee.Item("pay_type"), String) = "M", True, False))
                    Next
    ...
    Private Sub btnSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnSave.Click
        ...
        If Trim(txtDepartment.Text) = "" Then
            MsgBox("Please choose a department.", MsgBoxStyle.Information)
            txtDepartment.Focus()
            btnSave.Enabled = True
            Exit Sub
        End If
        ...
        Try
            ...
            mCommand.Parameters.AddWithValue("@employee_bio_id", txtCode.Value)
            ...
            mCommand.Parameters.AddWithValue("@pay_type", IIf(radDaily.Checked, "D", "M"))

            If mEmployee_ID = 0 Then
                mCommand.CommandText = "insert into employees (employee_bio_id,lastname,firstname,middlename,schedule_id,department_id,pay_type) values " & _
                                   "(@employee_bio_id,@lastname,@firstname,@middlename,@schedule_id,@department_id,@pay_type)"
                mCommand.ExecuteNonQuery()
                mEmployee_ID = CType(mCommand.LastInsertedId, Integer)
                mCommand.Parameters.AddWithValue("@employee_id", mEmployee_ID)
            Else
                mCommand.CommandText = "update employees set employee_bio_id = @employee_bio_id, " & _
                                    "lastname=@lastname,firstname=@firstname,middlename=@middlename, " & _
                                    "schedule_id=@schedule_id,department_id=@department_id,pay_type=@pay_type " & _
            ...
            If SysParam.AddEMployee = 1 Then
    ...
    Private Sub frmEmployeeInfo_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        Call Clear_Fields()
        If SysParam.AddEMployee = 0 Then
            btnSave.Enabled = False
        End If
